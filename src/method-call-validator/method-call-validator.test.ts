...
describe("MethodCallValidator", () => {
  it("can be instantiated", () => {
    ...
    expect(new MethodCallValidator(example)).toBeInstanceOf(
      MethodCallValidator
    );
  });
  ...
  it("validates methods that use by-name", () => {
    ...
    const methodCallValidator = new MethodCallValidator(example);
    const result0 = methodCallValidator.validate("foo", {
      foofoo: "123",
      barbar: "abc",
    });
    expect(result0).toBeInstanceOf(Array);
    expect(result0).toHaveLength(0);
    const result1 = methodCallValidator.validate("foo", {
      foofoo: 123,
      barbar: "abc",
    });
  ...
  it("unexpected reference error when the document has unresolved method reference passed", () => {
    ...
    expect(() => new MethodCallValidator(example)).toThrowError(
      MethodRefUnexpectedError
    );
  });
});
