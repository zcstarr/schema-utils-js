...
describe("applyExtensionSpec", () => {
  it("should successfully apply extension spec to meta schema", () => {
    ...
    expect(methodObjectDef.properties["x-notification"].summary).toBe(
      "OpenRPC Notification"
    );
  });
  ...
  it("should handle multiple extensions", () => {
    ...
    expect(methodObjectDef.properties["x-another-extension"].type).toBe(
      "string"
    );
  });
  ...
  it("should return unmodified schema when x-extensions is empty", () => {
    ...
    const result = applyExtensionSpec(
      emptyExtensionsDoc as OpenrpcDocument,
      metaSchema
    );
  ...
  it("should throw error when restricted schema definition doesn't exist", () => {
    ...
    expect(() => {
      ...
      "nonExistentDefinition does not exist, cannot apply extension x-notification"
    );
  });
